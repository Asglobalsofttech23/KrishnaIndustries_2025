...
module.exports = (db) => {
    ...
    router.get('/getleads', async (req, res) => {
        try {
            const query = 'SELECT * FROM leads_data ORDER BY QUERY_TIME DESC'; // Modify to match your table structure
...
//========================delete follow=======================================
router.delete('/deleteFollowingLead/:id', (req, res) => {
    const { id } = req.params;

    console.log('Delete request received for ID:', id); // Debugging

    const query = `DELETE FROM following_leads WHERE follow_id = ?`;
...
// =============================================================================================
    ...
    router.post('/waking-leads/create', async (req, res) => {
        const {
            emp_id,
            leads_name,
            leads_mobile,
        ...
        const existingQuery = `SELECT leads_mobile FROM walking_leads WHERE leads_mobile = ?`;
        db.query(existingQuery, [leads_mobile], (err, result) => {
            if (err) {
                console.error('Error checking existing lead:', err);
                return res.status(500).json({ message: 'Failed to check existing lead.' });
            }
            ...
            generateWakingLeadsId((leads_id) => {
                ...
                db.query(followingLeadsQuery, followingLeadsValues, (err) => {
                    if (err) {
                        console.error('Error inserting into following_leads:', err);
                        return res.status(500).json({ message: 'Failed to create following lead.' });
                    }
                    ...
                    db.query(walkingLeadsQuery, followingLeadsValues, (err) => {
                        if (err) {
                            console.error('Error inserting into walking_leads:', err);
                            return res.status(500).json({ message: 'Failed to create walking lead.' });
                        }
